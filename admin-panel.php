...
if(isset($_POST['app-submit']))
{
  ...
  // $con=mysqli_connect("localhost","root","","myhmsdb");
  //                   global $con;
  // $query = "select min_time, max_time from doctb where username='$doctor';";
  // $result = mysqli_query($con,$query);
  // if($result){
  //   $row = mysqli_fetch_assoc($result);
  //   $dr_min_time = $row['min_time'];
  //   $dr_max_time = $row['max_time'];
  // }


  // }
  // $dr_min_time = strtotime($dr_min_time);
  // $dr_max_time = strtotime($dr_max_time);
  ...
  if(date("Y-m-d",$appdate1)>=$cur_date){
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      // if ((date("H:i:s, $apptime1") <= date("H:i:s",$dr_max_time)) or date("H:i:s, $apptime1") >= date("H:i:s",$dr_min_time)) {
        // if($apptime >= $dr_min_time){
        
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
      // $apptime = date("H:i:s, $apptime1");
      // $appdate = date("Y-m-d", $appdate1);
        if(mysqli_num_rows($check_query)==0){
          $query=mysqli_query($con,"insert into appointmenttb(pid,fname,lname,national_id,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus) values($pid,'$fname','$lname','$nid','$gender','$email','$contact','$doctor','$docFees','$appdate','$apptime','1','1')");
        ...
        echo "<script>alert('We are sorry to inform that the time or date is already booked by some patient(s). Please choose different time or date!');</script>";
      }
    // }
    // else{
    //   echo "<script>alert('Kindly book within the doctor's available times. Please try again!');</script>";
    // }
  }
    else{
  ...
  if(isset($_POST['changepass']))
{
  if (isset($_SESSION['national_id'])){
      ...
      $pass = $_POST['password'];
      $query = "UPDATE patreg SET password = '$pass', cpassword= '$pass' WHERE national_id= '$nid';";
      $result = mysqli_query($con, $query);
...
function generate_bill(){
  ...
  while($row = mysqli_fetch_array($query)){
    ...
    <label> Doctor Name : </label>'.$dr.''.$row["doctor"].'<br/><br/>
    ...
    <label> Allergies Detected : </label>'.$row["allergy"].'<br/><br/>
    <label> Prescription : </label>'.$row["prescription"].'<br/><br/>
    <label> Fees Paid : </label>'.$row["docFees"].'<br/><br/>
    <div><i>Thank you for choosing us...</i></div>
    <label style="color:red;"><i>Your health our global priority<i/></label>'.'<br/><br/>
...
if(isset($_GET["generate_bill"])){
  ...
  $content .= '
      <br/>
      <h2 align ="center" style="color:blue;"><i class="fa fa-user-plus" aria-hidden="true"></i> Global Hospital</h2></br>
      <h3 align ="center" style="color:green;"> Bill </h3>
               ...
               <div class="col-sm-4" style="left: 5%">
                ...
                <div class="col-sm-4" style="left: 20%;margin-top:5%">
                  <div class="panel panel-white no-radius text-center">
                    <div class="panel-body" >
                      <span class="fa-stack fa-2x"> <i class="fa fa-square fa-stack-2x text-primary"></i> <i class="fa fa-list-ul fa-stack-1x fa-inverse"></i> </span>
                      <h4 class="StepTitle" style="margin-top: 5%;">Password</h2>
                    
                      <p class="cl-effect-1">
                        <a href="#list-change-pass" onclick="clickDiv('#list-change-pass-list')">
                          Change password
                        </a>
                      </p>
                    </div>
                ...
                <tbody>
                  <?php 
                    ...
                    while ($row = mysqli_fetch_array($result)){
                      ...
                      $spec = $row['spec'];
                      $time_available = $row['min_time']." - ".$row['max_time'];
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <!-- <a href="admin-panel.php?ID=" 
                              ...
                              <input type = "submit" onclick="alert('Bill retrieved Successfully');" name ="generate_bill" class = "btn btn-success" value="Generate Bill"/>
                              </a>
                              </td>
                              </form>
